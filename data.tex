...
As is generally true for a complicated particle physics experiment, the
reported PAO measurements depend not only on the intrinsic particle
...
energy.
One important simplification, however, is that virtually no background sources
...
the spatio-temporal development of the air shower which, in turn, allows
...
air shower. The PAO team analyzes the detector data and reports energy and
...
We consider the $N_C = 69$ UHECRs with energies $E \geq 55$~EeV cataloged in
...
direction, $\omega$ (a unit vector on the celestial sphere).  The detailed
shape of the likelihood contours is not simple (some contours are shown at
the PAO web site for lower-energy events), but they are roughly azimuthally
...
\enote{Issue:  EPS files (required by AoAS) do not support transparency,
so the ``darker tissots'' are not darker, etc..  A rasterized version may
be too large for arXiv.}

% Advice on rasterizing for arXiv:
...
\begin{figure}
\begin{centering}
%\includegraphics[angle=-90, width=.8\textwidth]{CR+LocalAGN-1-nobar.eps}\\
%\includegraphics[angle=-90, width=.8\textwidth]{CR+LocalAGN-2-nobar.eps}\\
%\includegraphics[angle=-90, width=.8\textwidth]{CR+LocalAGN-3.eps}
...
Fig.~\ref{fig:skymap}.
\enote{Revise to omit colorbars.}}
\label{fig:skymaps}
\end{figure}

\enote{Move the Fig 2 maps to appendix?}
...
close cosmic rays.  A concentration in the vicinity of the two closest AGN
